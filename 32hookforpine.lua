...
C.Notifications = {
	UIToggle = function(name, bool)
		if (C.UI.ShowUIMessages.Element:get()) then
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s%s%s was %s', col.Blue, col.White, col.Gold, name, col.White, toggleMsg))
			end
		end
	end,
			...
			for _, v in pairs(options) do
				if (#tab > 0) then
					...
					if (v == 'Chat Print') then
						C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s%s%s set to (%s)', col.Blue, col.White, col.Gold, name, col.White, tmpArr))
					else
						C.Notifications.Console.Log({text = cache.format('%s was set to (%s)', name, tmpArr), bool = true})
					end
				else
					if (v == 'Chat Print') then
						C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s%s%s was %sDisabled', col.Blue, col.White, col.Gold, name, col.White, col.Red))
					else
	...
	Votes = {
		Kick = function(tab)
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s The %s\'s started a vote to %s %s', col.Blue, col.White, col.Gold .. 'Vote' .. col.White .. ' -', teamFormatted, descriptionFormatted, targetFormatted))
			end
		end,
		Start = function(tab)
			local col = C.Colours
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s %s called a vote to %s', col.Blue, col.White, col.Gold .. 'Vote' .. col.White .. ' -', playerFormatted, descriptionFormatted))
			end
		end,
		Vote = function(tab)
			local col = C.Colours
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %sVote %s- %s voted %s', col.Blue, col.White, col.Gold, col.White, playerFormatted, voteFormatted))
			end
		end
	},
	Logs = {
		Hit = function(tab)
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] Hit %s\'s %s (fired: %s, hc: %s, dmg: %s) for: %s (%s) (%s hp)', col.Blue, col.White, nameFormatted, hitboxFormatted, firedAtHitboxFormatted, hitchanceFormatted, firedDamageFormatted, hitDamageFormatted, flags, healthFormatted))
			end
		...
		HitNormal = function(tab)
			if (not C.Vars.HitLog.AimHit) then
				...
				if (hasValue(options, 'Chat Print')) then
					C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] Hit %s\'s %s for %s dmg (%s hp remaining)', col.Blue, col.White, nameFormatted, formattedHitbox, hitDamageFormatted, hpRemainingFormatted))
				end
			end
		end,
		Miss = function(tab)
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] Missed %s\'s %s (r: %s, hc: %s, dmg: %s) (%s)', col.Blue, col.White, nameFormatted, hitboxFormatted, reasonFormatted, hitchanceFormatted, damageFormatted, flags))
			end
		end,
		TeamDamage = function(tab)
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s: %s hurt %s for %s (%s/%s)', col.Blue, col.White, col.Gold .. 'Team Damage' .. col.White, attackerFormatted, victimFormatted, dmgFormatted, totalDmg, maxDmg))
			end
		end,
		TeamKill = function(tab)
			...
			if (hasValue(options, 'Chat Print')) then
				C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s: %s killed %s (%s/3)', col.Blue, col.White, col.Gold .. 'Team Kill' .. col.White, attackerFormatted, victimFormatted, totalKills))
			end
		end
	},
	RankDump = function(tab)
		...
		if (hasValue(options, 'Chat Print')) then
			C.Libs.ChatPrint.Send(cache.format('[%s32hook%s] %s has %s wins (%s)', col.Blue, col.White, nameFormatted, winsFormatted, rankFormatted))
		end
	end,
	Console = {
		Help = function(tab)
...
C.UI = {
	LabelStart = {
		Element = cache.label(C.Config.Panel, C.Config.Side, '------------------[Start 32hook]----------------')
	},
	ShowUIMessages = {
	...
	LabelEnd = {
		Element = cache.label(C.Config.Panel, C.Config.Side, '-------------------[End 32hook]-----------------')
	}
}
